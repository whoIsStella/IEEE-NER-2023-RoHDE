...
from utils.util import gradient_penalty, initialize_weights, cross_correlation, gen_plot
        ...
        with tqdm(total=len(loader)) as pbar:
            ...
            with torch.no_grad():
                ...
                correlation = cross_correlation(
                    (last_real.detach().cpu().numpy().reshape(-1)),
                    (fake.detach().cpu().numpy().reshape(-1))
                )
                dtw, path = fastdtw(
                    (last_real.detach().cpu().numpy().reshape(-1)),
                    (fake.detach().cpu().numpy().reshape(-1)),
